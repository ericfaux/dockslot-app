...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: booking, error: bookingError } = await supabase
      ...
      .select(`
        *,
        trip_type:trip_types(id, title, description, deposit_amount, price_total),
    ...
    // Use the trip type's configured deposit amount (in dollars → convert to cents)
    // Falls back to 50% of total if trip type has no deposit set
    const depositAmount = booking.trip_type?.deposit_amount
      ? Math.round(booking.trip_type.deposit_amount * 100)
      : Math.round(booking.total_price_cents * 0.5);
    const isFullPayment = depositAmount >= booking.total_price_cents;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `${booking.trip_type.title} - ${isFullPayment ? 'Full Payment' : 'Deposit'}`,
      ...
      metadata: {
        ...
        isFullPayment: isFullPayment ? 'true' : 'false',
      },
    });
