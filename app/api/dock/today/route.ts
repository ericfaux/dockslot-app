...
import { DockTrip, DockWeatherData } from '@/app/dock/context/DockModeContext';
...
export async function GET(request: NextRequest) {
  ...
  if (!profile?.dock_mode_enabled) {
    return NextResponse.json({ error: 'Dock mode not enabled' }, { status: 403 });
  }
  ...
  return NextResponse.json({
    trips,
    weather,
    captainPhone: profile?.phone || null,
    captainName: profile?.full_name || profile?.business_name || null,
    timezone,
  });
}
